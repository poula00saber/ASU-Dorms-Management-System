    ...
    namespace ASUDorms.Infrastructure.Services
    {
        public class StudentService : IStudentService
        {
            ...
            public async Task<StudentDto> CreateStudentAsync(CreateStudentDto dto)
            {
                ...
                var dormLocationId = _authService.GetDormIdFromHeaderOrToken(); // Changed from async
            ...
            {
                _logger.LogDebug("🔍 GetAllStudentsAsync called");

                // For Registration users: Get from header
                // For Restaurant users: Will always be their fixed dorm
                var dormLocationId = _authService.GetDormIdFromHeaderOrToken();

                _logger.LogDebug("🎯 Selected dorm ID: {DormLocationId}", dormLocationId);

                if (dormLocationId == 0)
                {
                    _logger.LogWarning("⚠️ No valid dorm location, returning empty list");
                    return new List<StudentDto>();
                }

                // ✅ EXPLICITLY add both filters
                ...
                _logger.LogInformation("✅ Retrieved {Count} students for dorm {DormLocationId}",
            ...
            {
                var currentDormLocationId = _authService.GetDormIdFromHeaderOrToken();
